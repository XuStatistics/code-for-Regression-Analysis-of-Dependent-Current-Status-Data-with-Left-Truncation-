...
mal <- function(u,v,af){
  s1<-(-log(u))^af
  s2<-(-log(v))^af
  dcs<-exp(-(s1+s2)^(1/af))*(s1+s2)^(1/af-1)*(-log(v))^(af-1)/v
  dcs
}
    ...
    JJ = 1
      ...
      for(ct in 1:mj)
      {
        ...
        while(lok<n)
        {
          ...
          for(i in 1:n){
            temf = function(v){ dc(u=FT[i],v) - UC[i] }
           
            FC[i] = nleqslv(x=0.1, temf)$x
            if (FC[i]>1) FC[i]=1-1e-7
            if (FC[i]<0) FC[i]=1e-7
          }
      ...
      name=paste('Cox-Gumbel', 'n=', n, 'bata=', beta0, 'gamma=', gamma0, 'tau=', tau0, '.txt')
