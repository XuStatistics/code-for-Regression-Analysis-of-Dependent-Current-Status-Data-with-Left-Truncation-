...
copula<-function(u, v, af){
  log(1+(af^u-1)*(af^v-1)/(af-1))/log(af)
}


mal<-function(u, v, af){
  (af^u-1)*af^v/((af^u-1)*(af^v-1)+af-1)	
}
...
alltau0 = c(0.05, 0.1, 0.25, -0.05, -0.1, -0.25)  ## 
...
for(iii in 1:6)
{
  for(II in 1:2)
  {
    JJ = 1
      ...
      if(tau0 == 0.05) alpha0 = 0.6370458
      if(tau0 == 0.1) alpha0 = 0.4035852
      if(tau0 == 0.25) alpha0 = 0.0933005
      
      if(tau0 == -0.05) alpha0 = 1.5697457
      if(tau0 == -0.1) alpha0 = 2.477791
      if(tau0 == -0.25) alpha0 = 10.7180530
      ...
      for(ct in 1:mj)
      {
        ...
        while(lok<n)
        {
          ...
          UC = runif(nex)
          FC = log(1+UC*(alpha0-1)/(UC+(1-UC)*alpha0^FT))/log(alpha0)
      ...
      name=paste('Cox-Frank', 'n=', n, 'bata=', beta0, 'gamma=', gamma0, 'tau=', tau0, '.txt')
